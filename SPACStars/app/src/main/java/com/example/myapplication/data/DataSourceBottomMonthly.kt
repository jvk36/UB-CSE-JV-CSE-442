...
class DataSourceBottomMonthly {

    fun loadSPACs(): List<SPACBottomMonthlyPriceChange> {
        val list: MutableList<SPACBottomMonthlyPriceChange> = mutableListOf()
        val jsonArray = URL("https://sheets.googleapis.com/v4/spreadsheets/1D61Q4V_LwTXVCOedHkg-IROuZKTiJ25wg_qL75XvWlc/values/Monthly % Change!A2:E265?key=AIzaSyCZP2fBW638Gip01kDHMbHLaM84hWwU7uo").readText()
...
//        finalList.add(0, SPACLivePrices("TICKER", "LIVE PRICE", "COMPANY NAME"))
        ...
        for(i in 0..len) {
            ...
            list.add(i, SPACBottomMonthlyPriceChange(info.getJSONArray(i).getString(0),
                    info.getJSONArray(i).getString(1),
                    info.getJSONArray(i).getString(3).toFloatOrNull(),
                    company_name))
        }

        val finalList: MutableList<SPACBottomMonthlyPriceChange> = mutableListOf()
        ...
        finalList.sortBy { SPACBottomMonthlyPriceChange -> SPACBottomMonthlyPriceChange.stringResourceId3 }
