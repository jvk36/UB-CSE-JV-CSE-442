...
class ItemAdapterBottomDailyPriceChange(
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        if (thisdatapreloi[0] != "SPAC NOT FOUND") {
            PriceFunctions.onclicksetter_bottomdaily(holder, "Pre LOI", thisdatapreloi, context)
        } else if (thisdatadefagreement[0] != "SPAC NOT FOUND") {
            PriceFunctions.onclicksetter_bottomdaily(holder, "Definitive Agreement", thisdatadefagreement, context)
        } else {
            PriceFunctions.onclicksetter_bottomdaily(holder, "NOT_FOUND", arrayOf(item.stringResourceId1, item.stringResourceId3.toString().plus("%"), item.stringResourceId2), context)
        }
    }
