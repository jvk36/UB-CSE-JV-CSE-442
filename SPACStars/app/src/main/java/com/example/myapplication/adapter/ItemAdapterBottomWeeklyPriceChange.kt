...
class ItemAdapterBottomWeeklyPriceChange(
        private val context: Context,
        private val dataset: List<SPACBottomWeeklyPriceChange>,
...
) : RecyclerView.Adapter<ItemAdapterBottomWeeklyPriceChange.ItemViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
        ...
        val adapterLayout = LayoutInflater.from(parent.context)
                .inflate(R.layout.list_bottom_weekly_spac, parent, false)
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        if (thisdatapreloi[0] != "SPAC NOT FOUND") {
            PriceFunctions.onclicksetter_bottomweekly(holder, "Pre LOI", thisdatapreloi, context)
        } else if (thisdatadefagreement[0] != "SPAC NOT FOUND") {
            PriceFunctions.onclicksetter_bottomweekly(holder, "Definitive Agreement", thisdatadefagreement, context)
        } else {
            PriceFunctions.onclicksetter_bottomweekly(holder, "NOT_FOUND", arrayOf(item.stringResourceId1, item.stringResourceId3.toString().plus("%"), item.stringResourceId2), context)
        }
    }
